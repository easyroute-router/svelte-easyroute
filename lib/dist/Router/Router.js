...
var SilentModeService_1 = __importDefault(
  require('../Services/SilentModeService')
)
var Router = /** @class */ (function () {
  function Router(settings) {
    ...
    this.currentRouteData = Observable_1.default({
      params: {},
      query: {},
      name: ''
    })
    ...
    switch (this.mode) {
      ...
      case 'history':
        ...
        window.addEventListener('popstate', function (ev) {
          ev.state
            ? _this.parseRoute(
                PathService_1.default.stripBase(ev.state.url, _this.base),
                false
              )
      ...
      default:
        this.parser = new HashParser_1.default(this.routes)
        this.parseRoute(
          PathService_1.default.stripBase(window.location.hash, this.base) ||
            '/'
        )
        window.addEventListener('hashchange', function () {
          ...
          _this.parseRoute(
            PathService_1.default.stripBase(window.location.hash, _this.base)
          )
        })
        break
    }
  ...
  Router.prototype.getTo = function (matched, url) {
    var depths = matched.map(function (route) {
      return route.nestingDepth
    })
    var maxDepth = Math.max.apply(Math, depths)
    var currentRoute = matched.find(function (route) {
      return route.nestingDepth === maxDepth
    })
    if (!currentRoute) return null
    return Object.freeze(
      UrlParser_1.default.createRouteObject([currentRoute], url)
    )
  }
  Router.prototype.getFrom = function () {
    var current = this.currentMatched.getValue
    var depths = current.map(function (route) {
      return route.nestingDepth
    })
    var maxDepth = Math.max.apply(Math, depths)
    var currentRoute = current.find(function (route) {
      return route.nestingDepth === maxDepth
    })
    if (!currentRoute) return null
        ...
        switch (_b.label) {
          case 0:
            ...
            if (this.mode === 'history' && url.includes('#'))
              url = url.replace('#', '')
            matched =
              (_a = this.parser) === null || _a === void 0
                ? void 0
                : _a.parse(url.split('?')[0])
          ...
          case 1:
            allowNext = _b.sent()
            if (!allowNext) return [2 /*return*/]
            this.changeUrl(
              PathService_1.default.constructUrl(url, this.base),
              doPushState
            )
      ...
      return __generator(this, function (_a) {
        return [
          ...
          new Promise(function (resolve) {
            ...
            if (!_this.beforeEach) resolve(true)
            else _this.beforeEach(to, from, next)
          })
        ]
      })
    })
