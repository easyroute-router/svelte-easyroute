...
var HashBasedRouting = /** @class */ (function () {
  ...
  HashBasedRouting.prototype.parse = function (url) {
    ...
    var matchedRoutes = this.routes.reduce(function (total, current) {
      if (url.match(current.regexpPath)) total.push(current)
      return total
    }, [])
    var allMatched = []
    matchedRoutes.forEach(function (route) {
      allMatched = __spreadArrays(
        allMatched,
        BuildRoutesTree_1.getRoutesTreeChain(_this.routes, route.id)
      )
    })
    return uniqBy_1.default(allMatched, 'id')
  }
